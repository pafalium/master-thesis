...
Furthermore, the presented solutions include both desktop-based and cloud-based applications to allow for an understanding of what is currently possible in the cloud and how it compares to what is available on the desktop.
...
Rosetta's motivation is to allow architects to write portable \gls{gd} programs that generate equivalent models in any of their \gls{cad} applications.
...
Additionally, Rosetta also allows programs written in one programming language to use functionality defined not only in programs written in the same language, but also in programs written in other languages.
...
As opposed to the systems we talked about earlier, OnShape is a \gls{cad} tool for mechanical modeling and, as such, its users are familiar with \gls{cad} applications like SolidWorks\footnote{\url{http://www.solidworks.com/} (last accessed on 17/12/2016)} and Autodesk Inventor\footnote{\url{http://www.autodesk.com/products/inventor/overview} (last accessed on 17/12/2016)}.
...
The editing paradigms used in environments has been based on text editing, node graph editing, a combination of these two, or a combination of node graph and block-based editing.
