...
For example, they can use a faster but less functional \gls{cad} during exploration, and switch to a slower \gls{cad} when they need more information or when ready to move on to a later design phase.
...
As opposed to the previous systems, OnShape is a \gls{cad} tool for mechanical modeling and, as such, its users are familiar with \gls{cad} applications like SolidWorks\footnote{\url{http://www.solidworks.com/} (last accessed on 17/12/2016)} and Autodesk Inventor\footnote{\url{http://www.autodesk.com/products/inventor/overview} (last accessed on 17/12/2016)}.
