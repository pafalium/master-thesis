...
\footnote{\url{http://extempore.moso.com.au/} (last accessed on 10/10/2016)}
...
Finally, the way LightTable is structured is also relevant as it makes it possible to modify its behavior at runtime.
...
\subsection{Clara.io}
Clara.io\footnote{\url{https://clara.io/} (last accessed on 10/10/2016)}\cite{houston2013clara} is a cloud application for 3D modeling and animation, similar to desktop software like Blender\footnote{\url{https://www.blender.org/} (last accessed on 17/12/2016)} and 3ds Max.%
\footnote{\url{http://www.autodesk.com/products/3ds-max/overview} (last accessed on 17/12/2016)}
...
As opposed to the previous systems, OnShape is a \gls{cad} application for mechanical modeling and, as such, its users are familiar with \gls{cad} applications like SolidWorks\footnote{\url{http://www.solidworks.com/} (last accessed on 17/12/2016)} and Autodesk Inventor.%
\footnote{\url{http://www.autodesk.com/products/inventor/overview} (last accessed on 17/12/2016)}
...
Some environments have been implemented as desktop applications, while others have been implemented as web applications, as shown in Table~\ref{table:cloud:comp}.
...
In Table~\ref{table:cloud:comp} we can also see that all web applications, except Mobius, support remote storage directly.
...
As a result, there is no capable \gls{gd} environment in the web that is also capable of providing traceability, interoperability, and intuitive editing features.
