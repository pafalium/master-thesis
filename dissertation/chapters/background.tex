...
LightTable\footnote{\url{http://lighttable.com/} (last accessed 10/10/2016)} is a code editor for the Clojure programming language\cite{hickey2008clojure} and is an example of a desktop application that uses web technologies.
More specifically, it uses nw.js\footnote{\url{http://nwjs.io} (last accessed on 10/10/2016)} as its runtime, allowing it to use the html layout engine for its user interface, to use Javascript as its programming language, and to use node.js\cite{tilkov2010node} modules.
...
\subsection{Clara.io}
Clara.io\footnote{\url{https://clara.io/} (last accessed on 10/10/2016)}\cite{houston2013clara} is a cloud application for 3D modeling and animation, similar to desktop software like Blender and 3ds Max.
