...
To show this variety, we will present several examples that make use of some primitives that were implemented and also some programming techniques that they use.
...
In addition, we can also implement other types of buildings, like the one that {\tt coneBuilding} creates.
...
Nevertheless, it is possible to add support for them since there are JavaScript libraries that already implement \gls{csg}, like the case of OpenJSCAD's.
...
As seen in the chart, comparing export times (export AutoCAD) to normal running times (web page), export times are around 200x the normal running times for Mobius Truss, and 170x the normal running times for Ines Wall.
