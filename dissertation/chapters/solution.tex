...
To accomplish these tasks, there are two separate components, as seen in Figure~\ref{fig:archi:sol}: (1) the web application that support the \gls{ui} for creating programs; and (2) the remote CAD service, that offers an \gls{api} for running programs in \gls{cad} applications.
...
\label{sec:sol:page}
To handle the tasks of letting the architect develop programs, running programs, supporting program understanding, and displaying results, the web application's \gls{ui} has the layout shown in Figure~\ref{fig:page:view}.
...
\begin{figure}
  ...
  \caption{An example view of the web application's \gls{ui}.}
  \label{fig:page:view}
\end{figure}
...
g;
\end{minted}
\caption{An program creates spheres on the vertices of a cube.}
...
The last task that the environment needs to support is exporting to the most used commercial \gls{cad} applications.

As opposed to the other tasks, exporting to a \gls{cad} application requires the environment to communicate with other applications.
...
This application serves as a bridge between \gls{cad} applications installed in the computer and the environment running in the web application.
After being started, the application detects which \gls{cad} applications are installed in the computer and connects to the ``environment directory'' so it can be discovered by the web application.
When the architect needs to export a program to his \gls{cad} application, the environment connects to the application in his computer through the ``environment directory'' and starts to send requests to it, in order to create shapes in the \gls{cad} application.
...
The architect has created a program using the web application environment.
...
Note that it is only needed to install the application if and when the architect wants to get the program's results on a \gls{cad} application.
...
Instead of implementing the connection with \gls{cad} applications, the server uses Rosetta as an intermediate.
...
With this in mind, we chose to temporarily host the web application containing the \gls{ide} in the remote CAD application's server.
