...
it needs to let the architect develop programs; it needs to run programs; it needs to display their results; it needs to make it easy to see the traceability relationship between program and results; and it needs to export results to \gls{cad} applications.

To accomplish this, there are two separate components, as seen in Figure~\ref{fig:archi:sol}: (1) the web page, where the \gls{ui} for creating programs is; and (2) the remote CAD service, that offers an \gls{api} for running programs in \gls{cad} applications.
...
\section{Web page}
To handle the tasks of letting the architect develop programs, running programs, supporting traceability and displaying results, the web page has the layout shown in Figure~\ref{fig:page:view}.
...
To help with the implementation, we have used the THREE.js library\footnote{https://threejs.org/ (last accessed on 10/10/2016)}, to interact with WebGL, and the Ace Editor library\footnote{https://ace.c9.io (last accessed on 10/10/2016)}, to provide a syntax highlighted text editor.
...
\begin{itemize}
  \item {\bf Highlighting expressions and results} Every object that is part of the results of a program has been created by one expression of the program.
...
and (3) highlighting both pointed object and the corresponding creator expression, or both the pointed expression and the corresponding created results, as exemplified in Figure~\ref{fig:trace:example}.
...
In regard to detecting what the user is pointing at, we follow to approaches depending on the pointer being over the program or over the 3D view.
